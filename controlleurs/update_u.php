...
if (isset($_FILES["profile"]) && isset($_POST["nom"])&& isset($_POST["prenom"])&& isset($_POST["password"])&& isset($_POST["adress"])&& isset($_POST["age"])) {
	...
	$query=$db->prepare('UPDATE users SET nom=:nom, prenom=:prenom,age=:age, poste=:poste, password=:password, residence=:adress,tel=:numero, statut=:statut,profile=:profile,commentaire=:commentaire WHERE id=:id');
	$query->execute([
		"nom" =>$nom,
		"prenom" => $prenom,
		"age" =>$age,
