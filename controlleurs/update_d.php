...
if (isset($_FILES["profile"]) && isset($_POST["style"])&& isset($_POST["marque"])&& isset($_POST["quantite"])&& isset($_POST["qualite"])&& isset($_POST["couleur"])&& isset($_POST["anormalie"])&& isset($_POST["description"])&& isset($_POST["proprietaire"])) {
	...
	$query=$db->prepare('UPDATE vertements SET style=:style, marque=:marque,couleur=:couleur, qualite=:qualite, quantite=:quantite, anormalie=:anormalie,prixU=:prix, description=:description,image=:image,proprietaire=:proprietaire WHERE id=:id');
	$query->execute([
		"style" =>$style,
		"marque" => $marque,
		"couleur" =>$couleur,
